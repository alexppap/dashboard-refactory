...
| 职责 | 说明 |
|------|------|
| 事件绑定 | on(element, event, handler) — 绑定并记录 |
| 自动清理 | destroy() 时移除所有已绑定的事件监听器 |
| 解决泄漏 | 避免 removeEventListener 传入新匿名函数的问题 |
...
| # | 任务 | 说明 |
|---|------|------|
| P1-1 | 创建 `core/EventManager.js` | 提取事件绑定/清理逻辑，解决事件泄漏的根本问题 |
...
- 当前代码使用 PIXI v5/v6 API（`PIXI.Loader.shared`、`renderer.plugins.interaction`）
